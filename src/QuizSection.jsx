...
import {
  ...
  correctAnswers$,
  incorrectAnswers$,
  updateCorrectAnswersStat,
  updateIncorrectAnswersStat,
...
const QuizSection = ({ category }) => {
  ...
  const [incorrectAnswerStat, updateIncorrAnsState] = useState(
    incorrectAnswers$.value
  );
  const [correctAnswerStat, updateCorrAnsState] = useState(
    correctAnswers$.value
  );
  const [playedGamesStat, updateGames] = useState(playedGames$.value);
  const [correctPercentStat, updateCorrectPercentageStat] = useState(
  ...
  // TIMER LOGIC ===========================================
  useEffect(() => {
    if (!loaderActive && currentPage <= 10) {
  ...
  const checkAnswer = () => {
    ...
    const resultCorrectAnswer = allCorrectAnswer.filter((element) =>
      copyAnswers.includes(element)
    );
